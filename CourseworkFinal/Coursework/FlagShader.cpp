...
FlagShader::FlagShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"flagManipulation_vs.cso", L"flagManipulation_ps.cso");
}
...
void FlagShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void FlagShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture1, Light* light, Light* light2, Timer* time, float constantFactor, float linearFactor, float quadraticFactor, float amplitude, float frequency, float speed)
{

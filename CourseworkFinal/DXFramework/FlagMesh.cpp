...
FlagMesh::FlagMesh(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int lresolution)
{
...
void FlagMesh::initBuffers(ID3D11Device* device)
{
	VertexType* vertices;
	...
	// increment has been updated since original increment formula didn't map uv coords properly on small planes
	...
	for (j = 0; j<(resolution - 1); j++)
	{
		for (i = 0; i<(resolution - 1); i++)
		{
			...
			vertices[index].position = XMFLOAT3(positionX, positionY, 0.0f);
			vertices[index].texture = XMFLOAT2(u, v);
			vertices[index].normal = XMFLOAT3(0.0, 0.0, -1.0);
			indices[index] = index;
			index++;
